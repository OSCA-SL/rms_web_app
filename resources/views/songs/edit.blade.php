        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>
                        Edit Song
                        <small>Modify Song Data</small>
                    </h2>
                </div>
                <div class="body">
                    <form action="{{ route('songs.update', $song->id) }}" method="post" id="song-form">
                        @csrf
                        @method('PUT')
                        {{--<div class="ui-widget">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <div class="form-line ui-widget">
                                        ...
                                        <input type="text" id="title" name="title" class="form-control typeahead pb-2 mb-2" placeholder="Song Title" value="{{ $song->title  }}" />
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-12">
                                <div class="input-group date">
                                    <div class="form-line" id="bs_datepicker_container">
                                        <input name="released_at" id="released_at" type="text" class="form-control" placeholder="Released Date"  value="{{ $song->released_at  }}" >
                                    </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <input value="{{ $song->details  }}"  type="text" id="details" name="details" class="form-control typeahead pb-2 mb-2" placeholder="Details about this song (optional)" />
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="form-line">
                                        ...
                                        <select id="singers" name="singers[]" class="form-control show-tick select2" required multiple="multiple">
                                            @foreach($artists as $artist)
                                                <option {{ $song->isSinger($artist->id)?"selected":'' }} value="{{ $artist->id }}">{{ $artist->user->first_name }} {{ $artist->user->last_name }} - {{ $artist->membership_number }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <select id="music_directors" name="music_directors[]" class="form-control show-tick select2" required multiple="multiple">
                                            @foreach($artists as $artist)
                                                <option {{ $song->isMusician($artist->id)?"selected":'' }} value="{{ $artist->id }}">{{ $artist->user->first_name }} {{ $artist->user->last_name }} - {{ $artist->membership_number }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <select id="song_writers" name="song_writers[]" class="form-control show-tick select2" required multiple="multiple">
                                            @foreach($artists as $artist)
                                                <option {{ $song->isWriter($artist->id)?"selected":'' }} value="{{ $artist->id }}">{{ $artist->user->first_name }} {{ $artist->user->last_name }} - {{ $artist->membership_number }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <select id="producers" name="producers[]" class="form-control show-tick select2" required multiple="multiple">
                                            @foreach($artists as $artist)
                                                <option {{ $song->isProducer($artist->id)?"selected":'' }} value="{{ $artist->id }}">{{ $artist->user->first_name }} {{ $artist->user->last_name }} - {{ $artist->membership_number }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="row clearfix">
                            ...
                            <div class="col-sm-6 pull-right">
                                <button id="submit" type="submit" class="btn btn-primary btn-block btn-lg pull-right">Update</button>
                            </div>
...
{{--                    let size = Math.round('{{ $song->fileSize() }}' * 6 / 8);--}}
{{--                    // console.log(size);--}}

{{--                    // Create the mock file:--}}
{{--                    var mockFile = { name: "{{ $song->fileName() }}", size: size, status: Dropzone.QUEUED, accepted: true, upload: {}  };--}}
...
{{--                    myDropzone.options.maxFiles = myDropzone.options.maxFiles - existingFileCount;--}}
                    ...
                    fetch("{{ url()->to($song->file_path) }}")
                        .then(res => res.blob())
                        .then(blob => {
                            let file = new File([blob], "{{ $song->fileName() }}", blob);
